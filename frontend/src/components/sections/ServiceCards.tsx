...
export function ServiceCards({ data }: { data: ServiceCardsSection }) {
  ...
  const items =
    data.source === "live"
      ? (live.data ?? []).map((s) => ({ title: s.title, summary: s.summary, to: `/${brand}/services/${s.slug}`, icon: s.icon, illustration: undefined as string | undefined }))
      : (data.items ?? []);
    ...
    <SectionShell id={data.anchor}>
      ...
      <div className={cn("grid gap-6 md:grid-cols-2 lg:grid-cols-3", data.bento && "[grid-auto-flow:dense]")}>
        {items.map((s, i) => {
          ...
          const card = (
            <div className={cn("lift group h-full overflow-hidden rounded-lg border border-hairline bg-surface", featured ? "flex flex-col justify-end p-8" : "p-6")}>
              <ServiceArt name={s.illustration} featured={featured} />
