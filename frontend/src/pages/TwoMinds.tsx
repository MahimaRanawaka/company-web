...
export default function TwoMinds() {
  ...
  useEffect(() => {
    ...
    const map: Record<Side, React.RefObject<HTMLVideoElement>> = {
      ennobler: ennRef,
      oolo: ooloRef,
    };
  ...
  return (
    <div className="relative h-[100svh] w-full overflow-hidden bg-[#080B12] text-[#E8ECF1]">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 60% 50% at 50% 42%, rgba(40,60,90,0.30), transparent 70%)",
        }}
      />
      ...
      {!leaving && (
        <div className="absolute inset-0 z-10 grid grid-cols-2">
          {(["ennobler", "oolo"] as Side[]).map((s) => (
            <button
              key={s}
              aria-label={`Choose ${BRANDS[s].name} — ${BRANDS[s].kicker}`}
              onClick={() => choose(s)}
              onPointerEnter={() => !reduce && setFocus(s)}
              onFocus={() => setFocus(s)}
      ...
      {focus && !leaving && (
        <div
          className={`pointer-events-none absolute top-1/2 z-20 hidden -translate-y-1/2 px-2 md:block ${focus === "ennobler" ? "left-10 text-left lg:left-20" : "right-10 text-right lg:right-20"}`}
        >
          <div
            className="text-xs font-semibold uppercase tracking-[0.18em]"
            style={{ color: BRANDS[focus].accent }}
          >
            {BRANDS[focus].kicker}
          </div>
          <div
            className="mt-2 text-3xl font-bold tracking-tight"
            style={{ fontStyle: BRANDS[focus].italic ? "italic" : "normal" }}
          >
            {BRANDS[focus].name}
          </div>
          <div className="mt-2 max-w-[16rem] text-sm leading-relaxed text-[#cdd5df]">
            {BRANDS[focus].tagline}
          </div>
        </div>
      )}
            ...
            {(["ennobler", "oolo"] as Side[]).map((s) => {
              ...
              return (
                <button
                  ...
                  style={{
                    borderColor:
                      focus === s ? b.accent : "rgba(255,255,255,0.16)",
      ...
      {/* selection transition — portal video */}
      {leaving && (
